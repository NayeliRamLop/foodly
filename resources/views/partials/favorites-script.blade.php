<script>
    document.addEventListener('DOMContentLoaded', function () {
        const authUserId = Number(@json(optional(auth()->user())->id));
        const manageModal = document.getElementById('manageFavoriteRecipeModal');
        const manageForm = document.getElementById('recipeManageForm');
        const manageTitle = document.getElementById('manageFavoriteRecipeModalLabel');
        ...
        const filterForm = document.getElementById('favoriteFilterForm');
        ...
        filterForm.querySelectorAll('.filter-tab').forEach((button) => {
            button.addEventListener('click', function () {
                ...
                filterForm.querySelectorAll('.filter-tab').forEach((tab) => tab.classList.remove('active'));
                this.classList.add('active');

                filterForm.querySelectorAll('.filter-section').forEach((section) => {
                    section.classList.toggle('is-visible', section.getAttribute('data-panel') === panel);
                });
            });
        });

        filterForm.querySelectorAll('.filter-chip-row .filter-chip').forEach((button) => {
            button.addEventListener('click', function () {
        ...
        recipePreview = window.createRecipePreviewModalController({
            ...
            renderFooterActions: (recipe) => {
                if (Number(recipe.user?.id || 0) !== authUserId) {
                    return '';
                }
